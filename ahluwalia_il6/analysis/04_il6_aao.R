...
# 1) Load the datasets -------------------------------------------------
load(here::here("ahluwalia_il6", "data", "mr_data_aao.rda"))
...
# Prepare data ---------------------------------------------------------
df_trim <- df %>%
  # Remove rs660895 associated with rheumatoid arthirtis
  dplyr::filter(SNP != "rs660895")
...
df_trim <- df_trim_stat %>%
  # Remove rs660895 associated with rheumatoid arthirtis
  dplyr::filter(!SNP %in% unique(pleiotropic_snps))
...
save(
  ...
  file = here::here(
    "ahluwalia_il6", "analysis", "mr_results_aao.rda"
  )
)
