...
mr_fun <- function(data, input) {
  ...
  # # Example different values of PSI
  # psi_val <- seq(from = 0, to = 5, by = 0.02)
  # fits <- map(
  #   .x = psi_val,
  #   ~ MendelianRandomization::mr_conmix(inp, psi = .x)
  # ) %>%
  #   set_names(glue::glue("psi_{psi_val}"))
  #
  # conmix_res <- imap_dfr(
  #   .x = fits, .y = psi_val,
  #   ~ tibble::tibble(
...
# TwoSampleMR with PCA --------------------------------------------------
mr_pca <- function(betaXG, seXG, betaYG, seYG, rho) {

  assertive::assert_is_double(betaXG)
  assertive::assert_is_double(seXG)
  assertive::assert_is_double(betaYG)
  assertive::assert_is_double(seYG)

  Phi <- (betaXG/seYG) %o% (betaXG/seYG) * rho
  ...
  K <- which(cumsum(stats::prcomp(Phi, scale=FALSE)$sdev^2/sum((stats::prcomp(Phi, scale=FALSE)$sdev^2)))>0.99)[1]

  betaXG0 <- as.numeric(betaXG %*% stats::prcomp(Phi, scale = FALSE)$rotation[ ,1:K])
  betaYG0 <- as.numeric(betaYG %*% stats::prcomp(Phi, scale = FALSE)$rotation[ ,1:K])
  Omega <- seYG %o% seYG * rho
  pcOmega <- t(stats::prcomp(Phi, scale = FALSE)$rotation[ ,1:K]) %*% Omega %*% stats::prcomp(Phi, scale = FALSE)$rotation[ ,1:K]
  ...
  data.frame(
    est = as.numeric(beta_IVWcorrel.pc),
    se = as.numeric(se_IVWcorrel.fixed.pc)
  )
