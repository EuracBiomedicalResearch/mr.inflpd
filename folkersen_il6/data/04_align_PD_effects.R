...
load(here::here("folkersen_il6", "data", "tidied_instruments.rda"))
...
# Missing "rs9269909". Find proxy for it
proxy_query <- LDproxy(
  snp = "rs9269909",
  pop = "CEU",
  r2d = "r2",
...
# Look for proxies in PD dataset
pd_data_proxy <- pd_data |>
  filter(seqname %in% proxy_query$Coord)

# No good proxy is available in PD data. Remove that variants
...
# 8) Retrieve LD matrices ----------------------------------------------
ld_r2_001 <- ld_matrix(snps = df_r2_001$SNP, with_alleles = FALSE)
ld_r2_01 <- ld_matrix(snps = df_r2_01$SNP, with_alleles = FALSE)
...
save(
  ...
  file = here::here("folkersen_il6", "data", "mr_data_pd.rda")
)
