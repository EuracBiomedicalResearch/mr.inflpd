# Harmonized TNFalpha-PD data for MR analysis --------------------------
library(tidyverse)
...
# 2) Select instruments from PD data ----------------------------------
instr_nm <- instr_mr$chr_pos

pd_df <- pd_data %>%
  filter(chr_pos %in% instr_nm)

# Join TNFalpha and PD data -------------------------------------------
lj <- instr_mr %>%
...
save(
  mr_data,
  file = here::here("prins_tnfalpha", "data", "mr_data_pd.rda")
)
