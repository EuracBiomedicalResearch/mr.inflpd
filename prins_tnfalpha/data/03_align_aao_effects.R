# Harmonized TNFalpha-AAO data for MR analysis -------------------------
library(tidyverse)

# 1) Load the data -----------------------------------------------------
load(here::here("aao", "data", "aao2019_data.rda"))
load(here::here("prins_tnfalpha", "data", "tidied_instruments.rda"))
...
# 2) Select instruments from AAO data ----------------------------------
instr_nm <- instr_mr$chr_pos

aao_df <- aao_data %>%
  filter(chr_pos %in% instr_nm)

# Join TNFalpha and AAO data -------------------------------------------
lj <- instr_mr %>%
...
save(
  mr_data,
  file = here::here("prins_tnfalpha", "data", "mr_data_aao.rda")
)
